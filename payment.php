...
function applyCoupon() {
  ...
  const match = code.match(/^y22cm(\d{3})$/i);
  discountAmount = (match && parseInt(match[1]) >=1 && parseInt(match[1])<=215) ? 
                   Array.from(document.querySelectorAll('.book-select:checked'))
                        .reduce((sum, cb) => {
                          const book = books[cb.dataset.index];
